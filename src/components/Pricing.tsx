...
export default function PricingSection() {
  return (
    <>
          ...
          {additionalFees.map((item, index) => (
            <div key={index}
              ...
              <div className="grid grid-cols-[1fr_auto_auto] items-center gap-4">
                <span className="text-sm sm:text-base text-neutral-300 group-hover:text-neutral-200 transition-colors duration-200">{item.label}</span>
                <strong className="text-lg sm:text-xl text-neutral-200 text-right font-medium">{item.price}</strong>
                {item.unit && <small className="text-xs sm:text-sm text-neutral-400 whitespace-nowrap">{item.unit}</small>}
              </div>
            </div>
          ))}
        </div>
      ...
      <p className="text-center text-sm sm:text-base text-neutral-400 italic">
